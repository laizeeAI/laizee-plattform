package de.laizee.handler;

import de.laizee.handler.auth.ApiKeyAuth;
import de.laizee.handler.auth.Authentication;
import de.laizee.handler.auth.HttpBasicAuth;
import de.laizee.handler.auth.OAuth;
...
@Component("io.codejournal.maven.swagger2java.handler.ApiClient")
public class ApiClient {
    ...
    public <T> ResponseEntity<T> invokeAPI(String path,
        ...
        try {
            ...
            LOGGER.error("Connection to " + basePath + " failed!");
            System.exit(0);
        }
    ...
    private class ApiClientHttpRequestInterceptor implements ClientHttpRequestInterceptor {
        private final Logger API_LOGGER = Logger.getLogger(ApiClientHttpRequestInterceptor.class);
        ...
        private void logRequest(HttpRequest request, byte[] body) throws UnsupportedEncodingException {
            API_LOGGER.info("URI: " + request.getURI());
            API_LOGGER.info("HTTP Method: " + request.getMethod());
            API_LOGGER.info("HTTP Headers: " + headersToString(request.getHeaders()));
            API_LOGGER.info("Request Body: " + new String(body, StandardCharsets.UTF_8));
        }

        private void logResponse(ClientHttpResponse response) throws IOException {
            API_LOGGER.info("HTTP Status Code: " + response.getRawStatusCode());
            API_LOGGER.info("Status Text: " + response.getStatusText());
            API_LOGGER.info("HTTP Headers: " + headersToString(response.getHeaders()));
            API_LOGGER.info("Response Body: " + bodyToString(response.getBody()));
        }
